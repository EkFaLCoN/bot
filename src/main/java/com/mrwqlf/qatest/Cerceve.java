...
public final class Cerceve {
    ...
    public record Enstantane(double hedefCan) {}

    public Enstantane enstantane(LivingEntity hedef) {
        return new Enstantane(hedef == null ? -1 : hedef.getHealth());
    }

    /**
     * Sag tiktan sonra hedefe gercekten hasar gitti mi?
     * SADECE hedefin canina bakilir — oyuncunun cani dogal yenilenmeyle degistigi
     * icin ona bakmak yanlis pozitif uretiyordu.
     */
    public boolean etkiVarMi(Enstantane once, LivingEntity hedef) {
        return hedef != null && hedef.isValid() && once.hedefCan() >= 0
                && hedef.getHealth() < once.hedefCan() - 0.01;
    }

    /**
     * Her olcum icin TAZE hedef sart: onceki yetenegin kanamasi hala akiyorsa
     * yeni bir tetiklenme sanilir. Eski hedefleri siler, yenisini kurar.
     */
    public LivingEntity tazeHedef() {
        hedefleriTemizle();
        return yakinHedef();
    }

    /** Dogal can yenilenmesini durdurur — can emisi olcumunu bozmasin diye. */
    public void dogalYenilenmeKapat() {
        oyuncu.setFoodLevel(10);      // 18'in altinda dogal yenilenme calismaz
        oyuncu.setSaturation(0f);
    }

    public void dogalYenilenmeAc() {
        oyuncu.setFoodLevel(20);
        oyuncu.setSaturation(5f);
    }
