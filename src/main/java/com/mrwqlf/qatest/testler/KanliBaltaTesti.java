...
public final class KanliBaltaTesti implements Test {
    ...
    public void kur(Akis a, Cerceve c) {
        ...
        a.adim("can emis hazirla", x -> {
            x.hedefleriTemizle();
            x.dogalYenilenmeKapat();          // yoksa dogal yenilenme olcumu bozar
        ...
        a.adim("can emis baslangic", x -> oyuncuCanVurustanOnce = x.oyuncu().getHealth(), 5);
        ...
        a.adim("can emis olc", x -> {
            ...
            if (kazanc <= 0.01)
                x.rapor().acik("Can emisi CALISMIYOR — kanama hasarinin %35'i cana donmeli, oyuncunun cani hic artmadi "
                        + "(dogal yenilenme kapali olculdu, sonuc guvenilir)");
            else
        ...
        // KRITIK: ikinci denemeden once onceki yetenegin kanamasi (5 sn) bitmeli,
        // yoksa devam eden kanama "ikinci yetenek calisti" sanilir.
        a.adim("kanama sonlansin", x -> x.hedefleriTemizle(), 130);
        a.adim("yetenek 2 (bekleme suresi icinde)", x -> {
            yetenekHedef = x.tazeHedef();
            yetenekOnce = x.enstantane(yetenekHedef);
            x.sagTik();
        }, 60);
        a.adim("bekleme karari", x -> {
            if (yetenekHedef == null) return;
            if (x.etkiVarMi(yetenekOnce, yetenekHedef))
                x.rapor().acik("BEKLEME SURESI YOK — ilk tetiklemeden ~9 sn sonra yetenek yine etki uretti, "
                        + "config'deki 20 sn bekleme uygulanmiyor (taze hedefle olculdu)");
            else
        ...
        a.adim("kanama sonlansin 2", x -> x.hedefleriTemizle(), 130);
        ...
        a.adim("sinif kisiti karari", x -> {
            ...
            // sinif.silah.serbest izni (op varsayilani) kisiti tamamen bypass eder —
            // bunu "acik" diye raporlamak yanlis olur.
            if (x.oyuncu().hasPermission("sinif.silah.serbest")) {
                x.rapor().uyari(String.format(
                        "Sinif kisiti olculemedi: bu hesapta \"sinif.silah.serbest\" izni var (op varsayilani), "
                        + "kisit tasarim geregi asiliyor (buyucu %.1f / savasci %.1f). "
                        + "Gercek kisiti gormek icin op olmayan bir hesapla test et.", hasarBuyucu, hasarSavasci));
                return;
            }
